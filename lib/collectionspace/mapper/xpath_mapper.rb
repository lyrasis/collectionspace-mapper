...
module CollectionSpace
  module Mapper
    class XpathMapper
      ::XpathMapper = CollectionSpace::Mapper::XpathMapper
      ...
      def map(xpath)
        ...
        if xphash[:is_group] == false
          ...
          # todo: warn?
          if val_ct.size > 1
            rec_id = @data.fetch(Mapper::CONFIG[:rec_id_field].downcase, '')
            rhash = dhash.map{ |k, v| "#{k} (#{v.size} values)" }
            puts "WARNING: unequal number of values in record #{rec_id}: #{rhash.join(' -- ')}"
          end
