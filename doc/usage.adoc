...
[NOTE]
====
Given what the `DataHandler.validate` method's current behavior, this will work. However, if any functionality is ever built into that method such that it might be desirable to keep data from the `validate` response with the processed data, then it's probably safer to use Option 2.
====
...
If your `config[:response_mode]` was set to `verbose`, you will also receive the full data hashes from each stage of processing (`@orig_data`, `@combined_data`, etc.).
