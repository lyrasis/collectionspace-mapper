...
- `:transforms` - optional - Hash - Key (String)n is the data hash field to which the transforms should be applied. Value (Hash) is structured transformation instructions.
...
Once client, cache, config, and record mapper are set up, you will create a new DataHandler.
...
The DataHandler processes the record mapper to generate a blank XML document template, and creates a list of xpaths to be mapped with information about how to map the children of each xpath (as stand-alone fields? as members of a repeating field group?).
